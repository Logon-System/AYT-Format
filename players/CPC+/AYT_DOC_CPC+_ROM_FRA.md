...
## ATTENTION !
### Ce document concerne l'utilisation de la version **ROM** CPC+ de Ayt_builder
...
Sur **CPC+**, les **ROMS** peuvent être activées comme sur les **CPC** d'ancienne génération aux adresses **0x0000-0x3FFF** et/ou **0xC000-0xFFFF**.
Il est également possible de connecter une des 8 premières **ROM** aux adresses **0x4000-0x7FFF** ou **0x8000-0xBFFF**, mais cet adressage est inexploitable si la page ASIC doit être connectée.

Le player généré utilisera pour ses variables et pour la **liste DMA** un buffer passé en paramètre à *Ayt_Builder*

Sur les machines **"CPC+"** de Amstrad, il y a plusieurs paramètres liés aux spécificités techniques de ces machines, qui intègrent des **DMA** permettant de mettre à jour les registres **AY** *presque* automatiquement.

		ld ix,AYT_File		; AYT_File est l'adresse où se trouve le fichier AYT
		ld iy,AYT_Buffer	; Adresse d'un buffer en ram de 38 octets pour les variables du *player*
...
### Paramètre IY : Adresse d'un buffer en RAM 
Le registre IY doit contenir l'adresse d'un buffer de 38 octets initalisés à 0 (DS 38,0) (ou 36 octets si l'option **PlayerAccessByJP** vaut 1 (voir chapitre suivant)) .
...
- Vous ne devez pas placer le buffer en **0x41FE** car il va déborder dans la page de 256 octets suivante en **0x4200**.
