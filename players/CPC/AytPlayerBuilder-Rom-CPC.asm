...
;; ROM VERSION ROM VERSION ROM VERSION ROM VERSION ROM VERSION ROM VERSION ROM VERSION ROM VERSION ROM VERSION ROM VERSION ROM VERSION
;;============================================================================================================================================*
;; Mega Fucking Ultra Fucking Fast Player Fucking v2.1 for CPC :-) 
...
;; 25.11.2025 : New version exclusive for "Player in Rom"
;;
;;============================================================================================================================================
;; Performances
;; ------------
;; Option PlayerAccessByJP equ 0	; call player
;;     10 registers  : 380 nops (5.76 raster lines) / Player size : 305 bytes (*)
;;     11 registers  : 411 nops (6.25 raster lines) / Player size : 321 bytes (*)
;;     12 registers  : 442 nops (6.73 raster lines) / Player size : 337 bytes (*)
;;     13 registers  : 472 nops (7.20 raster lines) / Player size : 353 bytes (*)
;;     14 registers  : 501 nops (7.65 raster lines) / Player size : 369 bytes (*)
;;     Size of Ayt_Builder : 397 bytes
;;
;; Option PlayerAccessByJP equ 1	; jp player
;;     10 registers  : 366 nops (5.59 raster lines) / Player size : 299 bytes (*)
;;     11 registers  : 397 nops (6.07 raster lines) / Player size : 315 bytes (*)
;;     12 registers  : 428 nops (6.56 raster lines) / Player size : 331 bytes (*)
;;     13 registers  : 458 nops (7.03 raster lines) / Player size : 347 bytes (*)
;;     14 registers  : 487 nops (7.48 raster lines) / Player size : 363 bytes (*)
;;     Size of Ayt_Builder : 386 bytes
;;
...
;;--------------------------------------------------------------------------
;; Init AY Reg routine created at the end of the player. 
;;--------------------------------------------------------------------------
;; Notes :
...
;;              iy=address of RAM Buffer (7 bytes / 5 bytes if PlayerAccessByJP==0)
...
;;              bc=ptr on init routine to call juste once before playing via player address
;;
;;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
AYT_Builder_Start
		di
	...
	if PlayerAccessByJP
		...
		ld (Ayt_MusicCntValue),a	; Nb of loop for music (will be set by init routine)
...
Ayt_PtrFix_b1
		...
		exx 
	if PlayerAccessByJP
			ld hl,21+58+6		; part of constant time player jp mode
	else
			ld hl,29+61+9		; part of constant time player call mode
	endif
...
Ayt_MusicCnt equ $+1
		ld a,(0)		; 00/00/00/04/04/00 loop counter
...
Ayt_LoopSeq  equ $+1
		...
		pop hl			; 00/00/00/00/03/00
		push hl			; 00/00/00/00/04/00
		jr $+2			; 00/00/00/00/03/00
