...
// g++ -std=c++17 -O2 -pipe -s -DNOMINMAX ayt-player-cli.cpp -o ayt-player.exe -lmingw32 -lSDL2main -lSDL2
...
template <typename T>
static inline T maxi(T a, T b) { return (a > b) ? a : b; }
...
struct TAYTHeader {
    ...
    std::array<std::vector<uint8_t>, 16> Regs; // Registres [0..15][frame 0..FrameCount-1]
};
...
class TAYTParser {
    ...
    uint16_t ReadU16LE(size_t off) const {
        if (off + 1 >= FileData.size()) throw std::runtime_error("ReadU16LE: EOF");
...
public:
    void LoadFromFile(const std::string& path) {
        ...
        if (!F) throw std::runtime_error("Fichier introuvable: " + path);
        ...
        if (p + 3 < fileSize && FileData[p] == 0x00 && FileData[p+1] == 0x3F && FileData[p+2] == 0xBF) {
            p += 3;
        }
...
class TAYChip {
    ...
    void RecalcNoisePeriod() {
        NoisePeriod = maxi(1, int(R[6] & 0x1F));
    }
    void RecalcEnvPeriod() {
        EnvPeriod = maxi(1, int(R[0x0B] | (R[0x0C] << 8)));
    }
    void EnvReload() {
    ...
    void EnvStep() {
        if (EnvAtk) { if (EnvLevel < 31) ++EnvLevel; }
        else        { if (EnvLevel > 0)  --EnvLevel; }
        ...
        if (hitTop || hitBot) {
            ...
            if (EnvHold) return;
            if (EnvAlt)  EnvAtk = !EnvAtk;
            else         EnvLevel = EnvAtk ? 0 : 31;
        }
    }
...
public:
    TAYChip(bool UseYMTable, int /*MasterClock*/) {
        ...
        for (int i = 0; i < 32; ++i) DAC[size_t(i)] = src[i];
        Reset();
    }

    void Reset() {
        std::fill(std::begin(R), std::end(R), 0);
        for (int i = 0; i < 3; ++i) { TonePeriod[i]=1; ToneCnt[i]=0; ToneOut[i]=0; }
        NoisePeriod = 1; NoiseCnt = 0; NoiseLFSR = 0x1FFFF; NoiseOut = 1;
        EnvPeriod = 1; EnvCnt = 0; EnvShape = 0; EnvLevel = 0;
        EnvHold=false; EnvAlt=false; EnvAtk=false; EnvCont=false;
    }

    void WriteReg(int Index, int Value) {
        Index &= 0x0F; Value &= 0xFF; R[Index] = uint8_t(Value);
        switch (Index) {
            case 0: case 1: case 2: case 3: case 4: case 5: RecalcTonePeriods(); break;
            case 6: RecalcNoisePeriod(); break;
            case 0x0B: case 0x0C: RecalcEnvPeriod(); break;
            case 0x0D: EnvReload(); break;
            default: break;
        }
    }
    ...
    void Tick() {
        // tones
        for (int i=0;i<3;++i) if (ToneCnt[i]==0) ToneCnt[i]=TonePeriod[i];
        for (int i=0;i<3;++i) { if (--ToneCnt[i]==0) ToneOut[i]^=1; }

        // noise
        if (NoiseCnt==0) NoiseCnt = NoisePeriod;
        if (--NoiseCnt==0) {
            if ( ((NoiseLFSR ^ (NoiseLFSR>>3)) & 1u) != 0u ) NoiseLFSR = (NoiseLFSR>>1) | 0x10000u;
            else                                             NoiseLFSR = (NoiseLFSR>>1);
            NoiseOut = int(NoiseLFSR & 1u);
        }

        // envelope
        if (EnvCnt==0) EnvCnt = EnvPeriod;
        if (--EnvCnt==0) EnvStep();
    }

    void MixSample(float& LOut, float& ROut) {
        auto ChanEnabled = [](int regVal, int bit)->bool { return (((regVal >> bit) & 1) == 0); }; // 0=ON
        auto EnvLevelDAC = [this]()->int { return clampv(EnvLevel, 0, 31); };
        auto FixVolIdx   = [](int v)->int { v = clampv(v, 0, 15); return v*2 + 1; };
        ...
        for (int i=0;i<3;++i) {
            ...
            int chanOut = 1;
            if (gateTone)  chanOut &= ToneOut[i];
            if (gateNoise) chanOut &= (1 - NoiseOut);
            ...
            if (i==0)       mixL += c;
            else if (i==1) { mixL += c*0.5f; mixR += c*0.5f; }
            else            mixR += c;
        }
...
struct THighPass {
    ...
    void Process(double& L, double& R) {
        ...
        PrevInL = L; PrevOutL = yL;
        PrevInR = R; PrevOutR = yR;
        L = yL; R = yR;
    }
};
...
public:
        ...
        : P(Parser), AY(UseYMDAC, OverrideClock>0 ? OverrideClock : 1773400) // AYT n'a pas de MasterClock dans le fichier
    {
        ...
        FChipTicksPerSample = maxi(1, int(std::lround( (double(FMasterClock)/8.0) / double(FSampleRate) )));
        ...
        for (int i=0;i<frames;++i) {
            if (FApplyRegs) {
                if (FCurFrame >= FFrameCount) {
                    if (FLoopFrame < FFrameCount) FCurFrame = FLoopFrame;
                    else { 
                        FIsPlaying = false;
                ...
                // Boucle sur les 14 registres (0 à 13) car AYT ne s'occupe que de ceux-là
                for (int rr=0; rr<=13; ++rr) { 
                    uint8_t vreg = P->GetRegFrame(rr, FCurFrame);
                    if (rr==13 && vreg==0xFF) continue; // R13=0xFF est un skip (comme dans le code JS)
                    AY.WriteReg(rr, vreg);
                }
                FApplyRegs = false;
            }

            for (int t=0; t<FChipTicksPerSample; ++t) AY.Tick();

            float Lf=0.f, Rf=0.f; AY.MixSample(Lf, Rf);
...
static std::string GetArgValue(int argc, char** argv, const std::string& name, const std::string& defVal) {
    ...
    for (int i=1;i<argc;++i) {
        ...
        if (s[0]=='-') {
            ...
            if (eq) {
                std::string k(s+1, eq-(s+1));
                std::string v(eq+1);
                std::string kLOW = k; std::transform(kLOW.begin(), kLOW.end(), kLOW.begin(), ::tolower);
                std::string nameLOW = name; std::transform(nameLOW.begin(), nameLOW.end(), nameLOW.begin(), ::tolower);
                if (kLOW == nameLOW) return v;
            }
        }
    }
    return defVal;
